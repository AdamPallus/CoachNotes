# CoachNotes Documentation

## Current

- [User guide](user-guide.md): current user-facing setup and workflows.
- [UX anchor](ux-30-client-coach-story.md): the coach-centered product and workflow test.
- [Vercel deployment](deploy-vercel.md): proxy configuration, diagnostics, and production cutover.
- [Desktop release](release-desktop.md): validation, GitHub artifacts, and coordinated proxy/desktop releases.
- [Weekly client review](weekly-client-review.md): product behavior, data boundary, judgment rubric, and evaluation commands.
- [0.2.18 release notes](releases/v0.2.18.md): historical-note chronology and stronger longitudinal Weekly Review context.

## Future work

- [Hierarchical summary plan](hierarchical-summary-plan.md): possible future portfolio-scale summary architecture.

## Historical design records

These files explain prior decisions but do not describe the current implementation:

- [Liz feedback v1 spec](liz-feedback-v1-spec.md)
- [Liz feedback v1 status](liz-feedback-v1-status.md)
- [UI improvements](ui-improvements.md)
- [UI polish v2](ui-polish-v2.md)
